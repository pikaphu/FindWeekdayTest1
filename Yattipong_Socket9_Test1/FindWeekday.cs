...
namespace Yattipong_Socket9_Test1
{
	public class FindWeekday
	{
		...
		/// <summary>
		/// Call this function to find weekday
		/// </summary>
		public void DoFindWeekDay()
		{
            do
            {
                ...
                // 1. Get and check input date format
                ...
                // 2, Check date correction
                if (CheckInputDate(targetDate[0], targetDate[1], targetDate[2]) == false)
                {
                    Console.WriteLine("Error2!:");
                    continue;
                }

                // 3. Find weekday of input date
                GetWeekday(targetDate[0], targetDate[1], targetDate[2]);
		...
		public int[] GetInputDate()
		{
			...
			Console.Write("( *note. year must equal or more than 1900 ) \n :");
		...
		/// <param name="month"> input month as number (1 - 12) </param>
		/// <param name="year"> input year as number (1900) </param>
		public void GetWeekday(int day, int month, int year)
		{   /* FACT
			...
			*/
			...
			2. Then modulo the number with total week days we will get the day in week (<=7) [ex. (12 % 7) + 1 --> 6].  *+1 because we start day in week with 1 for Monday*
			...
			*/
            ...
            int dayInWeek = (sumOfDiffDays % 7) + firstDateWeekday;

            // 3. Compare day to enum
            string weekDayName = GetWeekDayName(dayInWeek);
        ...
        public int GetTotalDaysInMonth(int month, int year)
        {
            ...
            {
                // check February and leap years: 28 --> 29 days
                if (IsLeapYear(year))
                {
                    return 29;
                }
